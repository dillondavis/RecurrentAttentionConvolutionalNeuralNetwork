#!/bin/bash
# Usage:
# ./run_model_multi_scale.sh 3 1 vgg 30 ../data/CUBS 2
...
do
  for DATASET in CUBS; do
    ...
    mkdir $CKPT_DIR
    TAG='multi_'$ARCH'all_lr'$LR'_lrdecay'$LR_DECAY_EVERY'_'$RUN_ID'_epochs'$NUM_EPOCHS

    CUDA_VISIBLE_DEVICES=$GPU_ID python main_multi_scale.py --mode finetune --arch $ARCH \
      --dataset $DATASET --num_outputs ${NUM_OUTPUTS[$DATASET]} --batch_size 1 \
      --lr $LR --lr_decay_every $LR_DECAY_EVERY \
      --lr_decay_factor 0.1 --finetune_epochs $NUM_EPOCHS --scale $SCALE \
